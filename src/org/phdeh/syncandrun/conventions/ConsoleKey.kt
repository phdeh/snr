...
enum class ConsoleKey(val args: Int, val help: String = "") {
    ...
    KEY(1, "<KEY>  --  sets key to use this connection"),
    ...
    TIMEOUT(1, "<TIMEOUT>  --  sets connection timeout"),

    EXECUTE(1, "<COMMAND>  --  executes command in system shell"),

    UNQUIET(0, "  --  makes console messages")

    ;

    val shortKey = "-${name.filter { it.toLowerCase() !in setOf('e') }.first().toLowerCase()}"
